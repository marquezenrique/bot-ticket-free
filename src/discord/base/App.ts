...
type R<O extends BootstrapAppOptions> = O["multiple"] extends true
  ? Client[]
  : Client;
...
async function loadDirectories(
  ...
  const patterns: string[] = [
    ...
    directories
      .map((dir) => path.join(foldername, dir))
      .map((p) => p.replaceAll("\\", "/"))
      .map((p) => `./${p}/${pattern}`),
  ].flat();
  ...
  const versions = [
    ...
    "/",
    `${ck.hex("#68a063").underline("NodeJs")} ${ck.yellow(
  ...
  client.on("ready", async (client) => {
    ...
    if (options.commands?.guilds) {
      const guilds = client.guilds.cache.filter(({ id }) =>
        options?.commands?.guilds?.includes(id)
      );
    ...
    log.log(
      ck.greenBright(
        `➝ Online as ${ck.hex("#57F287").underline(client.user.username)}`
      )
    );
